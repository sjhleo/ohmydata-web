...
export default class DataModelService extends CommonService {
    ...
    public dataModel(queryModel: QueryModel<any>) {
        return this._post<any>(
            `${this.server}/unity/data-model/page`,
            queryModel
        );
    }
    ...
    public getModelById(id: any) {
        return this._get<any>(`${this.server}/unity/data-model/${id}`);
    }
    @serviceHandler({ title: "查询特定数据模型" })
    public deleteModelById(id: any) {
        return this._delete<any>(`${this.server}/unity/data-model/${id}`);
    }
    @serviceHandler({
        title: "保存数据模型"
    })
    public saveDataModel(model: DataModel) {
        if (model.id) {
            return this._put<any>(`${this.server}/unity/data-model`, model);
        } else {
            return this._post<any>(`${this.server}/unity/data-model`, model);
        }
    }
    @serviceHandler({ title: "删除数据模型" })
    public deleteDataModel(idArray: any) {
        return this._post<any>(`${this.server}/unity/data-model`, idArray);
    }

    @serviceHandler({ title: "解析请求参数" })
    public resolveParams(condition: any) {
        return this._post<any>(
            `${this.server}/unity/data-model/request-param`,
            condition
        );
    }
    @serviceHandler({
    ...
    public previewData(condition: any) {
        return this._post<any>(
            `${this.server}/unity/data-model/preview-expression`,
            condition
        );
    }
    @serviceHandler({
    ...
    public previewFinalData(condition: any) {
        return this._post<any>(
            `${this.server}/unity/data-model/preview`,
            condition
        );
    }
    ...
    public getMavonDoc(id: any) {
        return this._get<any>(`${this.server}/unity/data-model/${id}/render`);
    }
}
